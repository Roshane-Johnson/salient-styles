...
export class CreateGradientComponent implements OnInit {
	...
	ngOnInit(): void {
		const gradientAdminPreview = document.querySelector('#gradientAdminPreview') as HTMLElement;
	...
	createGradient(createGradientForm: FormGroup): void {
		...
		if (createGradientForm.valid) {
			this.http.post<ApiResponse>(`${environment.apiUrl}/gradient/create`, postBody).subscribe({
